    ...
    <aside
        ...
        <div class="h-19.5">
            ...
            <a class="block px-8 py-6 m-0 text-sm whitespace-nowrap text-slate-700" href="{{ url('admin/dashboard') }}">
                <img src="{{ asset('assets/img/yuma.png') }}"
                    ...
                    <div class="flex items-center md:ml-auto md:pr-4">
                        <form action="{{ route('admin.waiting') }}" method="GET">
                            <div
                                ...
                                <span
                                    class="text-sm ease-soft leading-5 absolute z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                                    <i class="fas fa-search" aria-hidden="true"></i>
                                </span>
                                <input type="text" name="search"
                                    class="pl-8.75 text-sm focus:shadow-soft-primary-outline ease-soft w-1/100 leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                    placeholder="Type here..." value="{{ request('search') }}" />
                                <button type="submit" class="hidden"></button>
                                <!-- Optional: Button for form submission -->
                            </div>
                        </form>
                    </div>
                        ...
                        <div
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <a href="{{ route('waiting.create') }}"
                                    class="text-xs font-semibold leading-tight text-slate-400">Tambah</a>
                            </div>
                        </div>
                                        ...
                                        @foreach ($waiting as $key => $item)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 text-center text-sm">
                                                    {{ $item->kuantitas_barang }}</td>
                                                <td
                                                    class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                    @if (isset($item->status))
                                                        @php
                                                            $status = strtolower(trim($item->status)); // Menghilangkan spasi dan membuat huruf kecil
                                                        @endphp
                                                        ...
                                                        @elseif($status == 'belum selesai')
                                                            <span
                                                                class="bg-gradient-to-tl from-red-500 to-black px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">{{ $item->status }}</span>
                                                        @elseif($status == 'dalam antrian')
                                                            <span
                                                                class=" bg-gradient-to-tl from-slate-600 to-slate-950 px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">{{ $item->status }}</span>
                                                ...
                                                <td
                                                    ...
                                                    <span
                                                        class="text-xs font-semibold leading-tight text-slate-400 ml-5">{{ $item->jenis_layanan }}</span>
                                                </td>
                                                <td
                                                    class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                    <span
                                                        class="text-xs font-semibold leading-tight text-slate-400">{{ $item->start_date }}</span>
                                                </td>
                                                <td
                                                    class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                    <span
                                                        class="text-xs font-semibold leading-tight text-slate-400">{{ $item->end_date }}</span>
                                                </td>
                                                <td
                                                    class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                    <a href="{{ route('waitings.edit', $item->id) }}"
                                                        class="text-xs font-semibold leading-tight text-slate-400">
                                                        Edit </a>

                                                    <form action="{{ route('waitings.destroy', $item->id) }}"
                                                        method="POST" class="delete-form" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button"
                                                            class="text-xs font-semibold leading-tight text-red-500 ml-4 delete-button">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
