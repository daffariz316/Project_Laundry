    ...
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200">
      ...
      <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" navbar-main navbar-scroll="true">
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            <!-- breadcrumb -->
            <h6 class="mb-0 mt-6 font-bold capitalize">Akun user</h6>
          </nav>
                    ...
                    <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                        <thead class="align-bottom">
                            <tr>
                                ...
                                <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xs tracking-wide whitespace-nowrap text-slate-400 opacity-70">Nomor HP</th>
                        ...
                        <tbody>
                            @foreach($users as $index => $user)
                                <tr>
                                    <td class="px-6 py-4 text-sm text-left">{{ $index + 1 }}</td>
                                    <td class="px-6 py-4 text-sm text-left">{{ $user->username }}</td>
                                    <td class="px-6 py-4 text-sm text-left">{{ $user->email }}</td>
                                    <td class="px-6 py-4 text-sm text-center">{{ $user->phone_number }}</td>
                                    <td class="px-6 py-4 text-sm text-center">{{ $user->password }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
